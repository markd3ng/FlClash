...
4. **For ja and ru**: provide real translations when confident; otherwise use the English translation as a placeholder (translators will fill these in later)
...
If the string contains dynamic content (e.g., `'创建 $name 配置'`), convert to the ARB ICU placeholder format:
- In ARB: `"keyName": "Create {name} config"`
- In Dart: `currentAppLocalizations.keyName(name)`

### Step 5: Regenerate localization code

After all ARB modifications, run:

```
dart run intl_utils:generate
```

This regenerates `lib/l10n/l10n.dart` and `lib/l10n/intl/messages_*.dart`. Skipping this step causes `undefined_getter` analyzer errors and CI build failures.
...
3. Verify with `flutter analyze 2>&1 | grep 'error •'` that no errors remain
...
- If a string contains dynamic content (variables, URLs), use ARB ICU placeholder syntax `{variableName}`
