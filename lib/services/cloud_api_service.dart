...
class CloudApiService {
  ...
  bool _shouldRetryWithProxy(DioException error) {
    return appController.isStart && _isNetworkIssue(error);
  }
  ...
  Future<Map<String, dynamic>> _request({
    ...
    try {
      return _handleResponse(
        await _primaryClient.post(endpoint, data: payload),
      );
    } on DioException catch (primaryError) {
      if (_shouldRetryWithProxy(primaryError)) {
        try {
          return _handleResponse(
            await _fallbackClient.post(endpoint, data: payload),
          );
  ...
  Future<CloudProfile> fetchUserProfile(
    String token, {
    bool isLoginAttempt = false,
  }) async {
    final response = await _request(
    ...
    return CloudProfile.fromApiResponse(
  ...
  Future<String?> fetchLatestVersion() async {
    try {
      final data = _handleResponse(
        await _primaryClient.get(_endpoints.version),
      );
...
class _ApiEndpoints {
  ...
  String get managedConfig => secrets.API_MANAGED_ROUTER;
